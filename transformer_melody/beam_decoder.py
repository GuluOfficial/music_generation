...
class Beam:
    ...
    def __init__(self, size, pad, bos, eos):
        ...
        self.scores = torch.zeros((size,), dtype=torch.float)
        self.all_scores = []
        ...
        self.next_ys = [torch.full((size,), self.PAD, dtype=torch.long)]
...
def beam_search(model, src, src_mask, max_len, pad, bos, eos, beam_size, device, first=True, previous_input=None):
    ...
    with torch.no_grad():
        ...
        if first:
            ...
            src = src.to(device)
            src_mask = src_mask.to(device)
            previous_input = previous_input.to(device)
            src_enc = model.dotattention(src, src_mask, previous_input)
