...
# Description: 乐谱生成模型：seq2seq_transformer的结构和flow based 模型（音符序列和时长序列）
import os
...
from VISinger.models_ae import SynthesizerTrn
...
from transformer_melody.MSE.mse_inference import generate_melody_mse, load_melody_model_mse
...
pretrained_visinger_path = "./VISinger/saved_models/G_AE.pth"
pretrained_visinger_path_f09 = "./VISinger/saved_models/G_AE_F09.pth"
pretrained_visinger_path_f19 = "./VISinger/saved_models/G_AE_F19.pth"

#pretrained_visinger_path = "./VISinger/saved_models/G_250000.pth"


class Synthesiser:
    def __init__(self, pretrained_model_path, hps_path="./VISinger/configs/ours_ae.json"):
    ...
    def process_input_data(self, text, notes, note_lengths):
        ...
        phonemes = self.text_normalizer.normalize(text, word_parse=False).replace(".", "").strip()#.replace(",", "SP")
        ph_seq = []
    ...
    def get_text(self, text):
        ...
        phonemes = phoneme_shengdiao.replace("1", "").replace("2", "").replace("3", "").replace("4", "").replace("5",
                                                                                                                 "").replace(",", "SP")
        phoneme_shengdiao = phoneme_shengdiao.replace(",", "SP")
...
syn_f09 = Synthesiser(pretrained_visinger_path_f09)
syn_f19 = Synthesiser(pretrained_visinger_path_f19)

fluid_synth = FluidSynth("./sound_fonts/MuseScore_General.sf2")
lyric2note, lyric2duration, lyric_dictionary, note_dictionary = load_melody_model_mse(device)
...
def syn_music(lyrics, singer_id=0):
    ...
    for lyrics in lyrics_list:
        # lyrics = lyrics.replace("飞向远方去飞翔", "永远陪在你身旁").replace("换你的梦", "呵护着你")
        if len(lyrics) < min_len:
            left += lyrics + ","
        else:
            left += lyrics
            new_list.append(left)
            left = ""
    lyrics_list = new_list

    lyrics_list = lyrics_list#[:3] 列表
    i = 0
    wavs = []
    all_notes = []
    previous_input = []
    sname = f"测试_{singer_id}"
    for k, lyrics in enumerate(lyrics_list):
        #此处lyrics是字符串
        print(lyrics)
        lyrics, midis, durations = generate_melody_mse(lyric2note, lyric2duration, lyric_dictionary, note_dictionary, lyrics, device, k, previous_input)
        # print(lyrics)
        # print(midis)
        # print(durations)
        if k > 0:
            previous_input = midis
        ...
        if singer_id == 1:
            wav = syn_f09.syn(text, wave_save_path)
        elif singer_id == 2:
            wav = syn_f19.syn(text, wave_save_path)
        else:
...
if __name__ == "__main__":
    lyrics = "test10|啊,你是我最心爱的姑娘AP|G#3 A3 A3 A3 A3 G#3 D4 D4 E4 E4 B3 rest|0.44 0.33 0.29 0.4 0.26 0.29 0.6 0.46 0.4 0.84 0.79 0.6"
    syn_music(lyrics, singer_id=-1)
