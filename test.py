...
def syn_music(lyrics):
    ...
    i = 0
    durations_list = [
        [2.261, 3.0, 3.397, 4.146, 4.521, 5.619,],
        [5.619, 6.006, 6.368, 7.143, 7.905, 8.625, 9.404, 10.517, 11.651,],
        [11.651, 12.375, 13.137, 13.493, 14.661,],
        [14.661, 15.01, 15.41, 16.134, 16.883, 17.606, 18.381, 19.286, 19.791,],
    ]
    k = 0
    delay_time = durations_list[0][0]
    ...
    while len(lyrics_list) > 0:
        ...
        # durations = [0.425, 0.766-0.425, 1.542-0.766, 1.905-1.542, 2.655-1.905,
        #              3.409-2.655, 3.791-3.409, 4.532-3.791, 5.267-4.532, 6.033-5.267, 6.782-6.033, 7.899-6.782, 8.658-7.899,
        #              9.406-8.658, 10.132-9.406, 10.909-10.132, 12.031-10.909,
        #              12.381-12.031, 12.763-12.381, 13.494-12.763, 14.277-13.494, 15.037-14.277, 15.420-15.037, 15.862-15.420, 16.862-15.862]

        print(pitches, durations)
        if end_pos < n:
            pitches += ["rest"]
            durations += [(notes[end_pos].start - notes[end_pos-1].end) / 1000.]
        else:
            pitches += ["rest"]
            durations += [1.0]

        durations = durations_list[k]
