...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True, infer=False, validate=False, reset=False, debug=False, use_default=False):
    if config == '':
        ...
        parser.add_argument('--hparams', type=str, default=hparams_str,
                            help='location of the data corpus')
    ...
    global hparams
    assert (args.config != '' or args_work_dir != ''), "args.config=%s and args_work_dir=%s" % (args.config, args_work_dir)
    saved_hparams = {}
    # 保存所有参数
